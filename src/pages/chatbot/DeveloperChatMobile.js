...
const DeveloperChatMobile = () => {
  const [messages, setMessages] = useState(initialMockMessages);
  const [input, setInput] = useState('');
  const [chatHistory, setChatHistory] = useState(() => {
    const saved = localStorage.getItem('chatHistory_mobile_app_design');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('chatHistory_mobile_app_design', JSON.stringify(chatHistory));
  }, [chatHistory]);
  ...
  const panelClass = 'mobile-app-chatbot-history-panel';
  const panelTitle = 'Mobile App Chatbot History';
  ...
  return (
    <div className="chatbot-layout">
      ...
      <div className={`chatbot-container developer-chat-card${historyOpen ? ' with-history' : ' full-width'}`}> 
        <Card className="chatbot-card developer-chat-card">
          <Card.Header className="d-flex align-items-center">
            <FaCode className="me-2" /> <span>Mobile App Design</span>
          </Card.Header>
      ...
      <div className={`history-panel${historyOpen ? '' : ' closed'}`}> 
        <div className="panel-header" style={{ marginTop: '34px' }}>
          <h3>{panelTitle}</h3>
          <button
...
export default DeveloperChatMobile; 